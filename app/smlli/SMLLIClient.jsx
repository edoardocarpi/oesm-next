...
function FontiSection() {
  return (
    <div style={{ padding: "48px 0 80px", borderTop: "1px solid var(--border)" }}>
      ...
      {/* Percorso verso misurazione diretta */}
      <div style={{ border: "1px solid var(--border)", padding: "24px 28px" }}>
        <div style={{ fontFamily: "var(--sans)", fontSize: 12, fontWeight: 600, letterSpacing: "0.1em", textTransform: "uppercase", color: "var(--text-muted)", marginBottom: 16 }}>
          Percorso verso la misurazione diretta
        </div>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 24 }} className="grid-3 smlli-roadmap">
          {[
            { fase: "Fase 1", periodo: "2025–2026", desc: "Dati pubblici disponibili e accordi informali con INSS, ISS, UPECEDS per statistiche aggregate non pubblicate." },
            { fase: "Fase 2", periodo: "2026–2027", desc: "Survey annuale sui frontalieri bidirezionali con campione stratificato per settore, qualifica e reddito." },
            { fase: "Fase 3", periodo: "2027+",     desc: "Accesso a microdati anonimi INSS per la stima diretta della massa salariale, superando le stime parametriche." },
          ].map((p, i) => (
            <div key={p.fase} style={{ borderTop: i === 0 ? `2px solid ${ACCENT_RED}` : "2px solid var(--border)", paddingTop: 16 }}>
              <div style={{ fontFamily: "var(--sans)", fontSize: 11, fontWeight: 500, color: i === 0 ? ACCENT_RED : "var(--text-muted)", marginBottom: 4 }}>{p.fase} · {p.periodo}</div>
