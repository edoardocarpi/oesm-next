...
const KPI_LABELS_EN = {
  ...
  labor_force:      'Labour force',
  inflation:        'Inflation (CPI)',
  exports_pct:      'Exports (% GDP)',
  imports_pct:      'Imports (% GDP)',
  trade_openness:   'Trade openness',
  govt_expenditure: 'Govt expenditure',
  tourism_arrivals: 'Tourist arrivals',
  tourism_receipts: 'Tourism receipts',
};

const SETTORI_EN = {
  'Manifattura':              'Manufacturing',
  'Servizi finanziari':       'Financial services',
  'Turismo e commercio':      'Tourism & trade',
...
function ChartCard({ indicator }) {
  ...
  const seo    = INDICATOR_SEO_EN[indicator.id];
  const href   = seo ? `/en/data/${seo.slug}` : '/en/data';
  const catLabel = CATS_EN[indicator.category] || indicator.category;
      ...
      <div className="chart-card-clickable" style={{ background: 'white', padding: '28px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: 'var(--mono)', fontSize: 9, color: 'var(--border-strong)', letterSpacing: '0.06em' }}>Data</span>
          </div>
        </div>
...
export default function DashboardClientEN() {
  const [tab, setTab] = useState('all');
  const categoryKeys = Object.keys(CATEGORIES);
  const tabs = ['all', ...categoryKeys];
  const filtered = tab === 'all' ? INDICATORS : INDICATORS.filter(i => i.category === tab);
  ...
  return (
    <div style={{ paddingTop: 64 }}>
        ...
        <div style={{ maxWidth: 1280, margin: '0 auto', padding: '0 40px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '20px 0 0', flexWrap: 'wrap', gap: 8 }}>
            <div>
              ...
              <h1 style={{ fontFamily: 'var(--serif)', fontSize: 26, fontWeight: 400 }}>Economic Dashboard</h1>
            </div>
          ...
          <div style={{ display: 'flex', gap: 0, marginTop: 20, overflowX: 'auto' }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setTab(t)} style={{ background: 'none', border: 'none', borderBottom: tab === t ? '2px solid var(--text-primary)' : '2px solid transparent', cursor: 'pointer', fontFamily: 'var(--sans)', fontSize: 12, fontWeight: tab === t ? 500 : 400, color: tab === t ? 'var(--text-primary)' : 'var(--text-muted)', padding: '10px 16px', whiteSpace: 'nowrap', letterSpacing: '0.04em', flexShrink: 0 }}>
                {t === 'all' ? 'All' : (CATS_EN[t] || CATEGORIES[t].label)}
              </button>
            ))}
          </div>
        </div>
      ...
      <div style={{ maxWidth: 1280, margin: '0 auto', padding: '40px 40px 0' }}>
              ...
              return (
                <Link key={kpi.id} href={href} style={{ textDecoration: 'none' }}>
                  <div style={{ background: 'white', padding: '20px 16px', cursor: 'pointer' }}>
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 9, fontWeight: 500, letterSpacing: '0.1em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 10 }}>{KPI_LABELS_EN[kpi.id] || kpi.label}</div>
        ...
        {/* Sectors */}
        {(tab === 'all' || tab === 'macroeconomia') && (
          <div style={{ background: 'white', border: '1px solid var(--border)', padding: '32px', marginBottom: 40 }}>
            <div style={{ marginBottom: 28 }}>
              <div style={{ fontFamily: 'var(--sans)', fontSize: 12, fontWeight: 500, marginBottom: 2 }}>GDP composition by sector</div>
              <div style={{ fontFamily: 'var(--mono)', fontSize: 10, color: 'var(--text-muted)' }}>% of total value added · 2023</div>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {SETTORI.map(s => (
                <div key={s.nome}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6, flexWrap: 'wrap', gap: 4 }}>
                    <span style={{ fontFamily: 'var(--sans)', fontSize: 13, color: 'var(--text-secondary)' }}>{SETTORI_EN[s.nome] || s.nome}</span>
        ...
        <div style={{ padding: '20px 0 60px', borderTop: '1px solid var(--border)' }}>
          <p style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.7, maxWidth: 700 }}>
            <strong style={{ fontWeight: 500, color: 'var(--text-secondary)' }}>Methodological note.</strong>{' '}
            Data updated via World Bank Open Data API. Historical series cover 2010–2023. Click any chart to access the full historical series.
          </p>
        </div>
      </div>
      <Footer />
