...
function KPICard({ kpi }) {
  ...
  const seo    = INDICATOR_SEO_EN[kpi.id];
...
const HERO_STATS = [
  { label: 'Years of data',      valore: '14' },
  { label: 'Monitored indicators', valore: '13' },
  { label: 'Institutional sources', valore: '5' },
];

const FEATURES = [
  { num: '01', title: 'Verified data',         desc: 'Collected from World Bank, IMF and San Marino institutions via official APIs.' },
  { num: '02', title: 'Automated pipeline',    desc: 'Weekly updates via GitHub Actions. No manual intervention required.' },
  { num: '03', title: 'Open access',           desc: 'All data is public and downloadable in CSV. No paywall.' },
  { num: '04', title: 'Scalable architecture', desc: 'Adding a new indicator requires a single line in the registry.' },
];
    ...
    <div>
        ...
        <div className="page-section" style={{ position: 'relative', maxWidth: 1280, margin: '0 auto', padding: '0 40px 80px', width: '100%' }}>
          <div style={{ opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(10px)', transition: 'opacity 0.6s ease, transform 0.6s ease', marginBottom: 32, marginTop: 48 }}>
            <span style={{ fontFamily: 'var(--mono)', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--accent)', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              Economic Observatory — San Marino
            </span>
          </div>
          ...
          <h1 style={{ fontFamily: 'var(--serif)', fontSize: 'clamp(44px, 7vw, 96px)', fontWeight: 300, lineHeight: 1.05, letterSpacing: '-0.02em', maxWidth: 900, marginBottom: 40, opacity: visible ? 1 : 0, transform: visible ? 'none' : 'translateY(20px)', transition: 'opacity 0.7s ease 0.1s, transform 0.7s ease 0.1s' }}>
            Economic data.<br />
            <span style={{ fontStyle: 'italic' }}>Rigorous analysis.</span><br />
            Open access.
          </h1>
          ...
          <p style={{ fontFamily: 'var(--sans)', fontSize: 16, fontWeight: 300, color: 'var(--text-secondary)', maxWidth: 520, lineHeight: 1.75, marginBottom: 48, opacity: visible ? 1 : 0, transition: 'opacity 0.7s ease 0.2s' }}>
            The Economic Observatory of San Marino collects, processes and publishes the Republic's main macroeconomic indicators. A structured, updated and open database for researchers, journalists and institutions.
          </p>

          <div className="hero-actions" style={{ display: 'flex', gap: 16, flexWrap: 'wrap', opacity: visible ? 1 : 0, transition: 'opacity 0.7s ease 0.3s' }}>
            <Link href="/en/economy-san-marino" className="btn-primary">Explore the Dashboard</Link>
            <Link href="/en/data" className="btn-outline">Access the Data</Link>
          </div>
      ...
      <section style={{ background: 'var(--off)', padding: '80px 40px', borderTop: '1px solid var(--border)', borderBottom: '1px solid var(--border)' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: 48, flexWrap: 'wrap', gap: 16 }}>
            <div>
              <p style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 8 }}>Key indicators — 2023</p>
              <h2 style={{ fontFamily: 'var(--serif)', fontSize: 32, fontWeight: 300 }}>Macroeconomic overview</h2>
            </div>
            <Link href="/en/economy-san-marino" className="btn-outline hide-mobile">Full dashboard</Link>
          </div>
          ...
          <div style={{ marginTop: 16, textAlign: 'center' }}>
            <span style={{ fontFamily: 'var(--sans)', fontSize: 11, color: 'var(--text-muted)' }}>
              Click an indicator to see the full historical series
            </span>
          </div>
        </div>
      </section>
          ...
          <div style={{ background: 'white', border: '1px solid var(--border)', borderLeft: '3px solid #b8272c', padding: '36px 40px', display: 'grid', gridTemplateColumns: '1fr auto', gap: 40, alignItems: 'center' }} className="grid-2">
            <div>
              ...
              <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.75, maxWidth: 560 }}>
                Approximately <strong style={{ color: '#b8272c', fontWeight: 600 }}>230 million euros</strong> in labour income leave the San Marino economy each year, spent in Italy by cross-border workers. Equivalent to 12% of GDP.
              </p>
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontFamily: 'var(--serif)', fontSize: 48, fontWeight: 300, color: '#b8272c', lineHeight: 1, marginBottom: 4 }}>–230</div>
              <div style={{ fontFamily: 'var(--sans)', fontSize: 12, color: 'var(--text-muted)', marginBottom: 20 }}>million EUR · 2024</div>
              <Link href="/en/indices/smlli" className="btn-outline" style={{ borderColor: '#b8272c', color: '#b8272c' }}>Explore the index</Link>
            </div>
          </div>
        </div>
      </section>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }} className="grid-2">
            <div>
              <p style={{ fontFamily: 'var(--mono)', fontSize: 10, letterSpacing: '0.14em', textTransform: 'uppercase', color: 'var(--accent)', marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
                <span style={{ display: 'inline-block', width: 16, height: 1, background: 'var(--accent)' }} /> The Project
              </p>
              ...
              <p style={{ fontFamily: 'var(--sans)', fontSize: 15, color: 'var(--text-secondary)', lineHeight: 1.8, marginBottom: 40 }}>
                OESM was created to provide a structured, transparent and regularly updated platform for monitoring San Marino's economy. Data is collected from verified institutional sources and updated automatically via ETL pipelines.
              </p>
              <Link href="/en/about" className="btn-outline">About the project</Link>
            </div>
            <div>
              {FEATURES.map((item, i) => (
                <div key={item.num} style={{ padding: '28px 0', borderTop: i === 0 ? '1px solid var(--text-primary)' : '1px solid var(--border)', borderBottom: i === 3 ? '1px solid var(--text-primary)' : 'none', display: 'flex', gap: 24 }}>
                  ...
                  <div>
                    <div style={{ fontFamily: 'var(--sans)', fontSize: 14, fontWeight: 500, marginBottom: 4 }}>{item.title}</div>
      ...
      <section style={{ background: 'var(--text-primary)', padding: '80px 40px' }}>
        <div style={{ maxWidth: 1280, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 32 }}>
          <div>
            <h2 style={{ fontFamily: 'var(--serif)', fontSize: 36, fontWeight: 300, color: 'white', marginBottom: 12 }}>Collaborate with OESM</h2>
            <p style={{ fontFamily: 'var(--sans)', fontSize: 14, color: 'rgba(255,255,255,0.6)', maxWidth: 480 }}>
              Are you a researcher, institution or journalist? Contact us for collaboration, data requests or contributions to the project.
            </p>
          </div>
